...
### ⬇ [**PA Toolbox v0.10.0 をダウンロード（APK・2.7MB）**](https://github.com/minetan1229/pa_system_app/raw/main/dist/pa-toolbox-v0.10.0.apk)
...
## 現在の状態: **39ツールすべて実装済み**（Phase 5 の課金・法務のみ未着手）— v0.10.0
...
| 6 | SE パッド（取り込み / オフライン再生 / 同時発音 / **画面をまたいで再生継続**） | 実装済み（実機未検証） |
| 6 | 本番モード（通知オフ・画面点灯・音声フォーカスを個別に選択） | 実装済み（実機未検証） |
| 6 | SPLメーター スイープ再生（20Hz→20kHz ログスイープを端末スピーカーから出力） | 実装済み |
| 6 | 本番タイマー 終了予定時刻の表示（カウントダウン中に "終了予定 HH:MM" を表示） | 実装済み |
| 6 | 本番進行 案件管理から進行表を取り込む / 初回起動時の3ステップ使い方ガイド | 実装済み |
| 6 | **今日の進行表を自動で読み込む**（案件の日付・搬入時刻を見て、本番画面とホームが同じものを出す） | 実装済み（実機未検証） |
| 6 | **ホームの「本番が始まっています／スタートする」** — 予定時刻を過ぎていると、開いた時点で気づける | 実装済み（実機未検証） |
| 6 | **本番万能コントローラーを1画面に作り直し**（タブ廃止。進行表は案件管理側で作る前提にした） | 実装済み（実機未検証） |
| 6 | **本番画面のハウリング検出を検出画面と同じ中身に**（帯域ごとの直し方まで、画面を離れずに開く） | 実装済み（実機未検証） |

39ツールの一覧は [ToolId.kt](core/model/src/main/kotlin/com/patoolbox/core/model/ToolId.kt) が唯一の定義。
検索はホームでそのまま打てて、結果はその場に出る。

ホームの並びは上から
**今日の進行表 → おすすめ（★を付けていればその道具）→ マイク校正と近道 → すべての道具 → まとめ（分類の入口）**。
**この並びはどの段でも同じ**にしてある。分類は「探し方の目次」であって開いたときに最初に読みたいものではないので、
全部を見せたあと、一番下に置いている。以前は初心者にだけ分類の入口を先に見せていたが、
目当ての道具に届くまで必ず一段深く潜ることになり、かえって迷子になっていた。
...
並び自体は共通で、変わるのは**説明の量とカードの密度**だけ。

| | ホーム | カード | 道具一覧 |
|---|---|---|---|
| 初心者 | 見出しの絵＋入門の4つを最上段に。「迷ったらこの順番で」は分類と一緒に一番下 | 説明文あり | 前提知識の要る道具に「中級」「上級」の札 |
| 中級者 | 見出しの絵＋おすすめ4つを最上段に | 説明文あり | 札なし |
| 上級者 | 絵を出さない。自動のおすすめも出さず、★を付けた道具だけを最上段に | 説明文を省いて詰める | 札なし |
...
feature/
  ...
  showtimer             本番タイマー
  showrunner            本番万能コントローラー。案件管理で作った進行表を読み込んで上から順に消化する。
                        SE連動・ハウリング検出（直し方まで）・本番モードを1画面に載せ、本番中に画面を持ち替えさせない
...
（この2モジュールで342テスト、リポジトリ全体で478テスト）。

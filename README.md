...
## 現在の状態: Phase 0〜3 完了、Phase 4 着手中（35ツール中21が実装済み）
...
| 4 | ハウリング検出 / 電源計算 / カバレッジ | 実装済み |
| 4 | SPLロガー / FFT / スペクトログラム / ディレイ実測 / 極性 / IR・RT60 / ステージプロット / ワイヤレス | 未着手 |
...
**数値の正しさは `core:dsp` と `core:calc` の JVM テストで担保している**（合わせて256テスト）。

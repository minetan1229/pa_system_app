...
internal object DocumentHelp {
    ...
    val TOPICS: List<HelpTopic> = listOf(
        ...
        HelpTopic(
            ...
            title = "本番万能コントローラー",
            summary = "進行表を読み込んでカウントダウンし、SE連動とハウリング検出まで1画面で見る道具",
            keywords = listOf(
                "進行表", "カウントダウン", "SE", "同期音源", "ループ", "本番",
                "ハウリング", "スペクトラム", "延長", "自動",
            ),
            sections = listOf(
                HelpSection(
                    heading = "進行表はここでは作らない",
                    body = """
                        項目を組むのは「案件管理」の進行表。あちらで日付・搬入時刻と
                        項目を作っておき、この画面はそれを**読み込んで進めるだけ**にしてある。
                        同じ表を2か所で直せると、どちらが本物か分からなくなるため。

                        当日になると、日付が今日の進行表を自動で読み込む。
                        自分で選び直したいときは「進行表を取り込む」から。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "予定の時刻を過ぎているとき",
                    body = """
                        読み込んだ進行表の時刻に現在時刻が当たっていると、
                        「予定では『◯◯』が始まっています」と出る。押すと、
                        予定の開始時刻からの経過ぶんを引いた残り時間で走り出す。

                        勝手には始めない。現場は予定より早くも遅くもなるので、
                        始めるかどうかを決めるのは人の側にしてある。
                        ホームにも同じ案内が出るので、アプリを開いた時点で気づける。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "押し・巻きと延長",
                    body = """
                        各項目には「18:30–18:35」のような時刻が付く。押し・巻きが出ると、
                        いま走っている項目から先の時刻が自動でずれる。

                        時間が足りなくなったら、延長のプリセットを押すとその分数が仮選択される。
                        +/− で微調整してから「延長する」を押すと、いまの項目に加算される。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "ハウリング検出（直し方まで同じ画面で）",
                    body = """
                        マイクを許可して「検出を始める」を押すと、いちばん長く鳴っている
                        1本を大きく出す。「直し方をくわしく見る」を押すと、同じ画面の中で
                        スペクトラム図・感度・並べ替え・1本ずつの履歴と、
                        その帯域で打つ手までが開く。

                        別の画面へ飛ばさないのは、本番中に飛ばされると戻ってこられないため。
                        一覧の既定は**累計で長く鳴った順**。潰したいのは一瞬の派手な発振ではなく、
                        鳴り続けて本番を壊す方だから。
                    """.trimIndent(),
                ),
                HelpSection(
                    ...
                    body = """
                        ...
                        進行表の行をタップするとSEを1つ紐付けられる。
                        紐付けた項目が始まると、設定した遅延（既定0秒）だけ待ってから自動で鳴る。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "この画面での変更は残らない",
                    body = """
                        当日その場で変えた持ち時間・順番・SE連動は、この画面を開いている間だけ。
                        次に使う進行表そのものは案件管理側に残るので、
                        繰り返し使う進行はあちらで直しておく。
                    """.trimIndent(),
                ),
            ),

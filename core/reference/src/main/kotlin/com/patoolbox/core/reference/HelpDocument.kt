...
internal object DocumentHelp {
    ...
    val TOPICS: List<HelpTopic> = listOf(
        ...
        HelpTopic(
            ...
            title = "本番進行コントローラー",
            summary = "タイトルと持ち時間を並べた進行表を上から順にカウントダウン。SEも同じ画面で鳴らす",
            keywords = listOf("進行表", "カウントダウン", "SE", "同期音源", "ループ", "本番"),
            sections = listOf(
                HelpSection(
                    heading = "本番タイマーとの違い",
                    body = """
                        本番タイマーは1本のカウントダウンだけを見る道具。
                        こちらは「MCコメント→1曲目→転換→2曲目」のように
                        複数の項目を順番に消化していく進行を追うための道具で、
                        項目ごとに持ち時間を変えられる。

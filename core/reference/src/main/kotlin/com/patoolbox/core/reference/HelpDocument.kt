...
internal object DocumentHelp {
    ...
    val TOPICS: List<HelpTopic> = listOf(
        ...
        HelpTopic(
            ...
            title = "本番万能コントローラー",
            summary = "進行表のカウントダウン・SE連動・ハウリング測定・スペクトラムアナライザを1画面にまとめた、本番中に持ち替えない道具",
            keywords = listOf("進行表", "カウントダウン", "SE", "同期音源", "ループ", "本番", "ハウリング", "スペクトラム", "延長"),
            sections = listOf(
                HelpSection(
                    heading = "3つのタブ",
                    body = """
                        「進行」「モニター」「SEパッド」の3タブに分けてある。
                        本番タイマーは1本のカウントダウンだけを見る道具として残し、
                        こちらは「MCコメント→1曲目→転換→2曲目」のように
                        複数の項目を順番に消化しながら、SE連動とモニターまで
                        画面を切り替えずに触れるようにした道具。

                        1本だけで足りる本番はタイマーを、
                        項目が複数あって順番に進めたい本番はこちらを使う。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "進行表の時刻表示と延長",
                    body = """
                        「開始予定時刻」を入れると、各項目に「18:30–18:35」のような
                        時刻が付く。押し・巻きが出ると、いま走っている項目から先の時刻が
                        自動でずれる。時刻を気にしない現場は空欄のままでよく、
                        その場合はこれまでどおり分数だけの表示になる。

                        カウントダウン中に時間が足りなくなったら、延長のプリセットを押すと
                        その分数が仮選択される。+/− で微調整してから「延長する」を押すと、
                        いま走っている項目にその場で加算される。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "SEパッドと同期音源、時刻連動の遅延",
                    body = """
                        パッドは SE パッド画面と同じ保存先を見ている。
                        どちらの画面で取り込んでも、もう片方にすぐ出てくる。

                        ループを付けたパッドは「同期音源」として扱う。
                        バッキング音源やクリックのように鳴らし続けたいものはループを付け、
                        単発の効果音はループを付けないまま使う。

                        進行表の項目ごとに「設定」からSEを1つ紐付けられる。
                        紐付けた項目が始まると、設定した遅延（既定0秒）だけ待ってから自動で鳴る。
                        遅延はプリセットからその場で選び直せるほか、後から値だけ変更もできる。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "モニター（ハウリング測定・スペクトラムアナライザ）",
                    body = """
                        本番タイマーのモニタと同じ仕組み。マイクを許可して開始すると、
                        レベルとスペクトラムを表示しながら、いちばん突出している
                        ハウリングを検出して知らせる。本番中に画面を離れず、
                        時間とSEを操作しながら同時に見られる。
                    """.trimIndent(),
                ),
                HelpSection(
                    heading = "予定は端末に残らない",
                    body = """
                        進行表はこの画面を開いている間だけ保持する。
                        アプリを終了すると消えるので、繰り返し使う進行は
                        本番の前に組み直す前提の道具になっている。
                    """.trimIndent(),
                ),
            ),
        ),
